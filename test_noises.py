...
def main():
    ...
    noise = gen_noise(images.shape, 55/255)
    noisy_images = images + noise.data.numpy()

    images = torch.FloatTensor(images)

    for i in range(images.size()[0]):
        psnr_val = psnr(images[i].data.numpy().astype(np.float32), noisy_images[i].astype(np.float32), data_range=1)
        print(f'PSNR:{psnr_val}')
        clean_img = images[i].data.numpy()*255
        clean_img = np.einsum('ijk->jki', clean_img.astype(np.float32)) 
        noisy_img = noisy_images[i]*255
